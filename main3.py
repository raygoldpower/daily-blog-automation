...
GOOGLE_REFRESH_TOKEN3 = os.environ.get("GOOGLE_REFRESH_TOKEN3", "")  # 블로그3 전용
BLOGGER_BLOG_ID3      = os.environ.get("BLOGGER_BLOG_ID3", "")       # 블로그3 전용
...
def build_prompt(topic):
    ...
    return (
        ...
        "1. 나의 상황 묘사로 시작 — 구체적인 장면, 시간, 감정 포함 (억지 공감 금지)\n"
...
def fetch_unsplash(keyword):
    ...
    try:
        ...
        r = requests.get(
            ...
            params={"query": keyword, "per_page": 5, "page": page,
...
def make_image_html(url, margin_top="32px"):
    return (
        f'<div style="text-align:center;margin:{margin_top} 0 28px 0;">'
        f'<img src="{url}" alt="관련 이미지" '
        f'style="max-width:100%;border-radius:10px;box-shadow:0 2px 12px rgba(0,0,0,0.10);"/>'
        f'</div>\n'
    )
...
def generate_post(topic):
    ...
    print(f"[Claude] 글 생성 중... 주제: {topic['symptom']}")
    ...
    # 최후 기본값 (날짜/카테고리 제외)
    if not title:
        title = f"{topic['category']} 이야기 — 직접 겪고 나서 알게 된 것"
...
def publish_to_blogger(title, html_body, category):
    ...
    url = f"https://www.googleapis.com/blogger/v3/blogs/{BLOGGER_BLOG_ID3}/posts/"
    payload = {
        "title": title,
        "content": html_body,
        "labels": [category],
...
# ============================================================
def main():
    print(f"=== 블로그3 자동 발행 시작 ({TODAY}) ===")

    topic = pick_topic()
    print(f"[선택된 주제] {topic['symptom']} / {topic['category']}")
